...
class clsCuotas
{
    public static function Registro($conexion, $tipo, $idclient,$cuotas,$moras,$monto,$tasa,$fecha_des){
        try {
            ...
            $fecha = date("Y-m-d");
            $query = $conexion->prepare("INSERT INTO por_cobrar( cliente, tipo, monto, cuotas, tasa, fecha, mora ) 
    ...
    public static function Validar($conexion, $tipo, $idclient,$cuotas,$moras,$monto,$tasa,$fecha_des){
        try {
            date_default_timezone_set('America/Lima');
            $fecha = date("Y-m-d");
            $query = $conexion->prepare("SELECT * FROM por_cobrar WHERE tipo = :tipo AND cliente = :idclient AND monto = :monto AND cuotas = :cuotas
